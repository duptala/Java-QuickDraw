...
public class CanvasController {
  ...
  protected static void configureSettings(
      String wordInput, int timeInput, int accuracyInput, Double confidenceInput) {
    ...
    EventHandler<ActionEvent> counterEventHandler =
        new EventHandler<ActionEvent>() {
          public void handle(ActionEvent event) {
            ...
            time++;
            timerLabel.setText("Time remaining: " + String.valueOf(countdownTime - time));
            ...
            if (ExtraGamesController.isChooseMystery()) {
              if (time == 30) {
                wordLabel.setText("First Character:   " + randomWord.substring(0, 1));
              }
              if (time == 40) {
                wordLabel.setText("First Two Character:   " + randomWord.substring(0, 2));
              }
            }
            ...
            if (place > 0 & place < (accuracy + 1)) {
              if (probabilities.get(place - 1) > confidence) {
                Profile.getProfile().addWin();
                Profile.getProfile().compareSetFastestTime(time);
                checkAllBadges(time);
                // Scene is changed to success scene
                finishButton.getScene().setRoot(SceneManager.getRoot(Page.SUCCESS));
                // Success's place is updated to CanvasController's prediction place
                SuccessController.startTimeline();
                // Programs tells the user what place they are using text to speech
                textToSpeech.speak(intToPlaceString(place));
                // Timelines stopped
                stopTimelines();
              }
            }
            if (time == countdownTime) {
              ...
              Profile.getProfile().addLosses();
              checkAllBadges(time);
              // the scene is changed to game over scene, and its place is updated
              wordLabel.getScene().setRoot(SceneManager.getRoot(Page.GAME_OVER));
              GameOverController.startTimeline();
              // Program tells the user what player what place they are using text to speech
              textToSpeech.speak(intToPlaceString(place));
              // Timelines stopped
              stopTimelines();
            }
  ...
  @FXML
  private void onDefinitionSwitch(ActionEvent event) {
    Button button = (Button) event.getSource();
    Scene scene = button.getScene();
    scene.setRoot(SceneManager.getRoot(Page.DEFINITION));
  }
  ...
  private String arrangePredictions(ArrayList<String> predictions, ArrayList<Double> probabilites) {
    ...
    // add the predictions on the canvas page
   ...
   *
   * @return int 1-10 of the place the machine learning thinks the word to be drawn is. 0 it isnt in
   *     the top 10
  ...
  /**
   * All Badges which have not been earned by the user is checked to see if it has been earned this
   * round
   *
   * @param timeTaken time taken to finish the last round
   */
  private void checkAllBadges(int timeTaken) {
    Profile profile = Profile.getProfile();
    ArrayList<Badge> badgesNotEarned = new ArrayList<Badge>();
    badgesNotEarned.addAll(profile.getBadgesNotEarned());

    // Every Badge not earned by the user is checked to see if it has been earned
    // this round

    for (int i = 0; i < badgesNotEarned.size(); i++) {
      Badge badge = badgesNotEarned.get(i);
      if (badge.isBadgeEarned(timeTaken)) {
        // If it has been earned, the badge is added to the users earned badges and will
        // not be
        // checked next time
        profile.addBadgeEarned(badge);
        profile.removeBadgeNotEarned(badge);
      }
    }
  }
}